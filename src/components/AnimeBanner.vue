...
<template>
  ...
  >
    <div class="relative aspect-[4/5] bg-primary-500/5">
      ...
      <div
        class="absolute top-3 right-2 bg-primary-500/90 backdrop-blur-sm px-3 py-1.5 rounded-md"
      >
    ...
    <div class="flex flex-col gap-4 p-4">
      <h3 class="text-lg font-semibold text-black line-clamp-2 min-h-[2em]">
        {{
          selectedAnime.titles.find((t) => t.type === "English")?.title ||
          selectedAnime.titles[0]?.title
        }}
      </h3>

      <div class="flex items-center gap-3 text-sm">
        <span class="px-2.5 py-1 bg-primary-500/10 text-primary-500 rounded-md">
          {{ selectedAnime.type }}
        </span>
